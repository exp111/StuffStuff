...
[
    ...
    [ 10, "You cast magic missile as a 5th-level spell." ],
    ...
    [ 46, "You cast levitate on yourself." ],
    ...
    [ 78, "You cast polymorph on yourself. If you fail the saving throw, you turn into a sheep for the spell's duration." ],
    ...
    [ 88, "You cast fly on a random creature within 60 feet of you." ],
    ...
    [ 92, "If you die within the next minute, you immediately come back to life as if by the reincarnate spell." ],
...
currentRollTable = "table1";

function setTable(name)
{
    currentRollTable = name;
}
...
function getSleepTime(num)
{
    return Math.pow(0.4, num + 0) * 1000;
}
...
function roll()
{
    ...
    let table = rollTables[currentRollTable];
    let max = table.length;

    let doRoll = () =>
    {
        let random = getRandomInt(max);
        value.innerText = table[random][0];
        text.innerText = table[random][1];
    }
    ...
    for (let i = 0; i < 15; i++)
    {
        setTimeout(() => {
            doRoll();
        }, getSleepTime(i)); // Start fast get slower
    }
}